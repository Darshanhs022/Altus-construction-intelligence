create or replace view vw_tower_progress_summary as
select
    s.project_code,
    s.tower_name,
    s.structure_completed_units,
    s.structure_total_units,
    s.structure_completion_pct,
    coalesce(sum(f.completed_units), 0) as finishing_completed_units,
    coalesce(sum(f.total_units), 0)     as finishing_total_units,
    round(
        coalesce(sum(f.completed_units), 0)
        / nullif(coalesce(sum(f.total_units), 0), 0) * 100,
        1
    ) as finishing_completion_pct,
    round(
        (s.structure_completed_units + coalesce(sum(f.completed_units), 0))
        /
        nullif(
            (s.structure_total_units + coalesce(sum(f.total_units), 0)),
            0
        ) * 100,
        1
    ) as overall_completion_pct
from vw_tower_structure_progress s
left join vw_tower_finishing_activity_progress f
  on f.project_code = s.project_code
 and f.tower_name   = s.tower_name
group by
    s.project_code,
    s.tower_name,
    s.structure_completed_units,
